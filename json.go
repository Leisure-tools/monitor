...
func (j JsonObj) AsArray() []any {
	...
	panic(fmt.Sprintf("%v is not an array", j.V))
}
...
func (j JsonObj) AsMap() map[string]any {
	if m, ok := j.V.(map[string]any); ok {
		return m
	} else if j.IsMap() {
		m := make(map[string]any, j.Len())
		for r := j.Value().MapRange(); r.Next(); {
			m[r.Key().String()] = r.Value()
		}
		return m
	}
	panic(fmt.Sprintf("%v is not a map", j.V))
}
